...
TEST(Tokenizer_CharacterReferenceAfterText) {
  HtmlTokenizer tokenizer(std::string("A&amp;B"));
  std::vector<Token> tokens = tokenizer.tokenize();
  ...
  EXPECT_STREQ(text.c_str(), "A&B");
}

TEST(Tokenizer_UnknownCharacterReferencePreservesText) {
  HtmlTokenizer tokenizer(std::string("A&bogus;B"));
  std::vector<Token> tokens = tokenizer.tokenize();
  ...
  EXPECT_STREQ(text.c_str(), "A&bogus;B");
}

TEST(Tokenizer_UnknownAttributeCharacterReferencePreservesText) {
  HtmlTokenizer tokenizer(std::string("<div title=\"A&bogus;B\">"));
  std::vector<Token> tokens = tokenizer.tokenize();

  EXPECT_EQ(tokens.size(), 2);
  EXPECT_TRUE(tokens[0].isStartTag());
  EXPECT_EQ(tokens[0].attributes.size(), 1);
  EXPECT_STREQ(tokens[0].attributes[0].value.c_str(), "A&bogus;B");
}
...
TEST(TreeBuilder_ElementCreation) {
  HtmlTreeBuilder builder;
  auto doc =
      builder.build(std::string("<div id=\"test\" class=\"container\"><span>Text</span></div>"));
