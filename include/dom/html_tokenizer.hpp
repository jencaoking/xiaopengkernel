...
inline HtmlTokenizer::HtmlTokenizer(std::string_view input) : input_(input) {
  currentToken_ = Token::makeEndOfFile();
}

inline HtmlTokenizer::HtmlTokenizer(const loader::ByteBuffer& input)
    : ownedInput_(reinterpret_cast<const char*>(input.data()), input.size()), input_(ownedInput_) {
  currentToken_ = Token::makeEndOfFile();
}
...
inline bool HtmlTokenizer::isAppropriateEndTag() const {
  return currentToken_.isEndTag() && toLower(currentToken_.name) == toLower(lastStartTag_);
}
...
inline void HtmlTokenizer::handleMarkupDeclarationOpenState() {
  ...
  if (position_ + 6 < input_.size() && toLower(input_[position_]) == 'd' &&
      toLower(input_[position_ + 1]) == 'o' && toLower(input_[position_ + 2]) == 'c' &&
      toLower(input_[position_ + 3]) == 't' && toLower(input_[position_ + 4]) == 'y' &&
      toLower(input_[position_ + 5]) == 'p' && toLower(input_[position_ + 6]) == 'e') {
    for (int i = 0; i < 7; i++) {
      consumeNextInputCharacter();
    }
  ...
  if (allowCdata_ && position_ + 6 < input_.size() && input_[position_] == '[' &&
      input_[position_ + 1] == 'C' && input_[position_ + 2] == 'D' &&
      input_[position_ + 3] == 'A' && input_[position_ + 4] == 'T' &&
      input_[position_ + 5] == 'A' && input_[position_ + 6] == '[') {
    for (int i = 0; i < 7; i++) {
      consumeNextInputCharacter();
    }
...
inline void HtmlTokenizer::handleAfterDoctypeNameState() {
  ...
  if (toLower(c) == 'p' && position_ + 5 < input_.size() && toLower(input_[position_]) == 'u' &&
      toLower(input_[position_ + 1]) == 'b' && toLower(input_[position_ + 2]) == 'l' &&
      toLower(input_[position_ + 3]) == 'i' && toLower(input_[position_ + 4]) == 'c') {
    for (int i = 0; i < 5; i++) {
      consumeNextInputCharacter();
    }
  ...
  if (toLower(c) == 's' && position_ + 5 < input_.size() && toLower(input_[position_]) == 'y' &&
      toLower(input_[position_ + 1]) == 's' && toLower(input_[position_ + 2]) == 't' &&
      toLower(input_[position_ + 3]) == 'e' && toLower(input_[position_ + 4]) == 'm') {
    for (int i = 0; i < 5; i++) {
      consumeNextInputCharacter();
    }
...
inline std::string HtmlTokenizer::consumeCharacterReference() {
  ...
  const size_t startColumn = column_;
  auto restoreStart = [&]() {
