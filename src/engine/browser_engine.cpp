...
void BrowserEngine::loadHTML(const std::string &html) {
  ...
  // Submit parse work to the background thread вЂ?returns immediately
  m_pendingParse =
...
// в”Ђв”Ђв”Ђ Async Parse Completion (Main Thread) в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
...
void BrowserEngine::onParseComplete(ParseResult result) {
  ...
  std::cout << "[Engine] Parse complete вЂ?integrating results on main thread"
            << std::endl;
  ...
  // Register DOM into script engine (main thread only вЂ?QuickJS not
  // thread-safe)
...
// в”Ђв”Ђв”Ђ Script Execution (Main Thread) в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ

void BrowserEngine::executeScripts(
    const std::vector<dom::HtmlParser::ScriptData> &scripts) {
  for (const auto &script : scripts) {
    // Skip async and defer scripts вЂ?they are handled separately
...
// в”Ђв”Ђв”Ђ URL Resolution & Resource Fetching в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
...
// в”Ђв”Ђв”Ђ Layout (Main Thread) в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ

void BrowserEngine::buildLayout() {
  std::cout << "[Engine] Building Render Tree..." << std::endl;
  m_needsLayout = false;
  if (!m_document)
    return;

  // First build Render Tree with computed styles
  m_renderTree.build(m_document, m_stylesheet);

  // Build traditional Layout Box tree (backward compatibility)
  // We still use the existing LayoutTreeBuilder but keep RenderTree for dirty tracking
  ...
  auto layoutRoot = builder.build(bodyList[0]);
  if (layoutRoot) {
    ...
    algo.layout(layoutRoot, viewport);

    // Now assign layout boxes to render tree objects
    // and synchronize bounding boxes
    std::function<void(layout::LayoutBoxPtr)> assignToRenderTree = 
        [&](layout::LayoutBoxPtr box) {
          if (box->node()) {
            if (auto renderObj = m_renderTree.findRenderObject(box->node())) {
              renderObj->setLayoutBox(box);
            }
          }
          for (auto &child : box->children()) {
            assignToRenderTree(child);
          }
        };
    assignToRenderTree(layoutRoot);
    
    // Synchronize bounding boxes from layout to render tree
    m_renderTree.syncBoundingBoxesFromLayout();

    auto &dim = layoutRoot->dimensions();
...
// в”Ђв”Ђв”Ђ Event Loop в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ
...
int BrowserEngine::run() {
  ...
  while (running) {
    ...
    if (m_state == EngineState::Ready) {
      ...
      auto layoutRoot = m_renderTree.layoutRoot();
      if (m_needsPaint && layoutRoot) {
#ifdef ENABLE_OPENGL
        if (m_openglCanvas) {
          m_openglCanvas->clear(renderer::Color::White());
          m_painter.paint(layoutRoot, *m_openglCanvas);
          m_window.drawOpenGLTexture(m_openglCanvas->getTexture());
        }
#else
        if (m_canvas) {
          m_canvas->clear(renderer::Color::White());
          m_painter.paint(layoutRoot, *m_canvas);
...
void BrowserEngine::handleMouseClicks() {
  ...
  for (const auto &click : clicks) {
    if (click.button == window::MouseButton::Left) {
      auto hitNode =
          layout::HitTest::test(layoutRoot, static_cast<float>(click.x),
